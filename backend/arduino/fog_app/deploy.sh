...
#     bash backend/arduino/fog_app/deploy.sh          copy + restart + show logs
#     bash backend/arduino/fog_app/deploy.sh copy     copy only
#     bash backend/arduino/fog_app/deploy.sh logs     show logs only
#     bash backend/arduino/fog_app/deploy.sh stop     stop the app
...
if [ "$action" = all ] || [ "$action" = copy ]; then
    ...
    # The device API (what the phone app talks to) lives in backend/api; ship it next to main.py.
    for file in device_server.py streaming_detector.py cadence.py demo_page.html; do
        "$ADB" push "$APP_DIR/../../api/$file" "$REMOTE/python/" | tail -1
    done
