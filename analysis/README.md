...
| `check_device_server.py` | pass/fail check of the device server's cue safety (STOP holds, a stale STOP names no event, status carries the cue, the buzzer takes over when the phone leaves, no debug route), against a deterministic fake sensor |
